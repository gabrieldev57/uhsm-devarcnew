...
export async function GET_CensusMembers(ARC_FormCensus) {
    console.log('memberAssignment: ', ARC_FormCensus.memberAssignment);
    const response = await ARC_FormCensus.omniRemoteCall({
        input: { 'memberAssignment': ARC_FormCensus.memberAssignment },
        sClassName: "ARC_FormCensusControllerSpinOff",
        sMethodName: "GET_CensusMembersPerCensus",
        options: "{}",
    }, true);
 ...
 */
export function removeMember(census_members, member_index, census_errors) {
    ...
    census_members = updatePrimaryCopyAddressVisibility(census_members);
...
export function updatePrimaryCopyAddressVisibility(census_members){
    ...
    census_members.forEach(member => {
        member.DISABLE_PRIMARY_ADDRESS_AND_COPY_ADDRESS = !isUniquePrimary
    });
...
export function updateEmailAndPhoneVisibility(member) {
    ...
    if (!isChild || (isChild && isAdult)) {
        ...
        member.individual_errors = member.individual_errors.filter(
            (error) => !(
                error.name.includes('validatePhoneNumberFormat') ||
                error.name.includes('validateEmail')
            )
        );
    }
