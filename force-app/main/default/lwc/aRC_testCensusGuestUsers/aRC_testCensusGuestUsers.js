...
import template from './aRC_testCensusGuestUsers.html'
...
export default class arc_Census extends insOsCensus {
    ...
    connectedCallback() {
        ...
        console.log(JSON.stringify(this.osData));
    ...
    saveMembers(needRecalculation) {
        console.log("entro al savemembers")
        let medicalPlan;
        let visionPlan;
        let dentalPlan;

        console.log('requiredFieldMissing' + this.requiredFieldMissing);
        if (this.requiredFieldMissing == false) {
            ...
            if (editedMembers.length > 0) {
                ...
                console.log("JSON.stringify(this.IPInput)");
                console.log(JSON.stringify(this.IPInput));
                const params = {
                ...
                .then((response) => {
                    ...
                    this.loadCensus().then(loadResponse => {
                        if (needRecalculation == true) {
                            ...
                            this.handleUpdateCensusLoad(newCensusMap);
                            this.omniApplyCallResp(
                                {
        ...
        console.log('Census Final Update ' + JSON.stringify(this.census));
    }
    ...
    handleUpdateCensusLoad(newCensusMap) {
        console.log("entro al handleUpdateCensusLoad")
        // const saveResponse = saveMembersResponse;
        ...
        console.log(JSON.stringify('CENSUS' + this.census));
        this.calculateCensusInfo();
    }
    // Updates member information in census.
    handleUpdate(ev) {
       console.log("entro al handleUpdate")
    ...
    deleteMembers(memberIdentifiers) {
        console.log('CENSUS CON HANDLE' + JSON.stringify(this.census));
        let deletionIds = [];
        ...
        if (deletionIds.length > 0) {
            ...
            .catch((error) => {
                ...
                console.log('Census Final Delete ' + JSON.stringify(this.census));
        } else {
...
// if (JSON.stringify(this.SavedMembers) == JSON.stringify(this.employees)) {
    ...
    //  console.log("entro al requiredFieldsValidation")
        ...
        if (tempRequiredFieldsMissing.length > 0) {
            ...
            if(this.osData.afterCensus == false){
                console.log("UPS ERROR EN REQUIRED!")
                this.omniApplyCallResp(
                    {
                        'requiredFieldMissing': false,
                    }
