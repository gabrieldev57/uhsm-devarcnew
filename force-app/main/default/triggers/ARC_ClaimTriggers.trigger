trigger ARC_ClaimTriggers on Claim (before insert, before update, after insert, 
    ...
    if(Trigger.isBefore){
        if (Trigger.isDelete) {
            ClaimLiabilityChangeHandler.preventLiabilityDeletion(Trigger.old);
        } else {
            // Only run these when NOT deleting (when Trigger.new is available)
    ...
    if (Trigger.isAfter) {
        ...
        if (Trigger.isUpdate) {
            ...
            ClaimLiabilityChangeHandler.handleUpdate(Trigger.new, Trigger.oldMap);
        } else if (Trigger.isInsert) {
            ARC_ClaimTriggersHandler.clmCovForClmWithoutInput(Trigger.new);
            ClaimLiabilityChangeHandler.handleInsert(Trigger.new);
        } else if (Trigger.isUndelete) {
            ClaimLiabilityChangeHandler.handleUndelete(Trigger.new);
        }
    }
}